package com.wzdq.fengcai.utils.crash;
...
public class CrashHandler implements Thread.UncaughtExceptionHandler {
    ...
    private static final String PATH = Environment.getExternalStorageDirectory().getPath() + File.separator + "FengCai" + File.separator + "app" + File.separator + "CrashHandler" + File.separator + "log" + File.separator;
...
//            Process.killProcess(Process.myPid());
    ...
    private boolean handleException(Throwable ex) {
        ...
        new Thread() {
            ...
            public void run() {
                Looper.prepare();
                Toast.makeText(mContext, "很抱歉,程序出现异常,正在重新启动APP", Toast.LENGTH_SHORT).show();
                Looper.loop();
            }
        }.start();
        return true;
...
//                    });
    ...
    private void dumpExceptionToSDCard(Throwable ex) throws IOException {
        ...
        if (!Environment.getExternalStorageState().equals(Environment.MEDIA_MOUNTED)) {
            if (DEBUG) {
                LogUtils.d(TAG, "SD 卡不能存在或无法使用，跳过保存异常信息");
                return;
            }
        }
        ...
        try {
            ...
            LogUtils.e(TAG, "dump crash info failed,将crash信息导入SD卡失败");
        }
    }
